...
(function() {
    ...
    function generateBusinessInfo() {
        ...
        const businessSurnames = [
            ...
            'Adams', 'Baker', 'Bennett', 'Brooks', 'Campbell', 'Carter', 'Collins', 'Cook', 'Cooper', 'Cox', 'Cruz', 'Edwards', 'Evans', 'Fisher', 'Flores', 'Foster',
            'Gomez', 'Gonzalez', 'Gray', 'Hall', 'Hernandez', 'Hill', 'Howard', 'Hughes', 'Jenkins', 'Kelly', 'King', 'Lewis', 'Long', 'Lopez', 'Martin', 'Mitchell',
            'Morgan', 'Morris', 'Murphy', 'Myers', 'Nelson', 'Nguyen', 'Ortiz', 'Parker', 'Patterson', 'Perez', 'Perry', 'Peterson', 'Phillips', 'Powell', 'Price',
            'Ramirez', 'Reed', 'Reyes', 'Reynolds', 'Richardson', 'Rivera', 'Roberts', 'Robinson', 'Rodriguez', 'Rogers', 'Ross', 'Russell', 'Sanchez', 'Sanders',
            'Stewart', 'Sullivan', 'Thomas', 'Torres', 'Turner', 'Walker', 'Ward', 'Watson', 'White', 'Williams', 'Wood', 'Young',
            'Erikson', 'Goldberg', 'Kaplan', 'Nakamura', 'O\'Brien', 'Patel', 'Schmidt', 'Yamamoto', 'Zimmerman'        ];
  ...
  const residentialAddressDatabase = [
    ...
    async function fillForm() {
                 ...
                 for (const field of invalidFields) {
                    ...
                    if (field.type === 'tel' || field.name?.toLowerCase().includes('phone') || field.placeholder?.toLowerCase().includes('phone')) {
                        ...
                        switch (section) {
                             ...
                             case 'personal': default: basePhone = personalInfo.phone; break;
                        }
                        ...
                        if (!fixed) { // Last resort: simple digits
                         ...
                         if (stillInvalid) {
                            ...
                            switch (section) {
                                ...
                                case 'personal': default: originalEmail = personalInfo.email; break;
                            }
     ...
     function scrollToSubmitButton() {
        // Try CAPTCHA first
        const captchaElement = document.querySelector('div[data-drupal-selector="edit-captcha"], div.captcha-type-challenge--recaptcha, div.captcha, div[class*="captcha" i], div.g-recaptcha');
    ...
    function createUI() {
        ...
        const protectStatement = document.createElement('span');
        protectStatement.textContent = '🏳️‍⚧️ Protect Trans Youth! 🏳️‍⚧️'; // Added Emojis
        protectStatement.style.fontWeight = 'bold'; // Make the statement bold
        protectStatement.style.color = '#FFFFFF'; // Brighter white color
        protectStatement.style.fontSize = '16px'; // Larger font size
        // Optional: Add a subtle text shadow for more pop
        // protectStatement.style.textShadow = '0 0 3px rgba(91, 192, 222, 0.7)';
        protectStatementContainer.appendChild(protectStatement);
        ui.appendChild(protectStatementContainer); // Append first

        // Add VPN reminder (and Drag Handle) - [MODIFIED EMOJI & COLOR]
        const vpnReminder = document.createElement('div');
        vpnReminder.textContent = '🛡️ VPN ACTIVE? 🛡️'; // Changed Emojis
        vpnReminder.style.fontWeight = 'bold';
        vpnReminder.style.color = '#52b9e9'; // Changed Color
        ...
        loginLabel.style.cssText = labelStyle;
        const loginValue = document.createElement('span');
        loginValue.textContent = userLogin;
        ...
        buttonContainer.style.gridTemplateColumns = '2fr 1fr 1fr'; // Ratio for buttons
        ...
        goToTargetButton.style.cssText = buttonBaseStyle;
        goToTargetButton.style.backgroundColor = '#6f42c1'; // Purple base color
        ...
        buttonContainer.appendChild(resetButton);        ui.appendChild(buttonContainer);// Make UI draggable via VPN bar
    ...
    function updateStatus(customStatus) {
        ...
        if (statusValueEl) {
            ...
            if (customStatus && customStatus.toLowerCase().includes('error')) {
                 statusValueEl.style.color = '#ff7b7b'; // Red for errors
            } else if (customStatus && (customStatus.toLowerCase().includes('captcha') || customStatus.toLowerCase().includes('warning') || customStatus.toLowerCase().includes('validation'))) {
                 statusValueEl.style.color = '#f0ad4e'; // Orange for warning/captcha/validation
            } else if (customStatus && (customStatus.toLowerCase().includes('filled') || customStatus.toLowerCase().includes('success'))) {
                 statusValueEl.style.color = '#5cb85c'; // Green for success/filled
            } else if (customStatus && (customStatus.toLowerCase().includes('filling') || customStatus.toLowerCase().includes('generating') || customStatus.toLowerCase().includes('navigating'))) {
                 statusValueEl.style.color = '#5bc0de'; // Blue for in-progress actions
            } else {
                 statusValueEl.style.color = '#ddd'; // Default color (Ready)
            }
        }
    }
